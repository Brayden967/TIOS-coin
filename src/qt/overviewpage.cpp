...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::TIOS )
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("TIOS ")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeTIOScoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTIOScoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTIOScoinAmount = strAnonymizeTIOScoinAmount.remove(strAnonymizeTIOScoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTIOScoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTIOScoinAmount * COIN) nMaxToAnonymize = nAnonymizeTIOScoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTIOScoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTIOScoinAmount));
        strAnonymizeTIOScoinAmount = strAnonymizeTIOScoinAmount.remove(strAnonymizeTIOScoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTIOScoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
