...
#if defined(NDEBUG)
#error "TIOScoin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 60 * COIN;
    } else if (nHeight <= 74881 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 15 * COIN;
    } else if (nHeight <= 105122 && nHeight >= 74882) {
        nSubsidy = 5 * COIN;
    } else if (nHeight >= 105123) {
        nSubsidy = 3 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
{
    RenameThread("tioscoin-scriptch");
